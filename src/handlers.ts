...
const matchString = (target: string, matchers: Array<string | RegExp>) => (
  matchers.some(matcher =>
    typeof matcher === 'string' ? target.includes(matcher) : matcher.test(target)
  )
)

const senderOf = (message: Message) => {
  const room = message.room()
  return (room && room.alias(message.from())) || message.from().alias() || message.from().name()
}

const printMessage = (message: Message, content?: string) => {
  const room = message.room()
  if (room) {
    debug('[M] (%s) %s: %s', room.topic(), senderOf(message), content || message.content())
  } else {
...
const takeCareOf = async (baby: string, message: Message, replies: Array<string>) => {
...
export const createMessagesHandler = (config: Config) => (message: Message) => {
  ...
  debug('room && room.alias(message.from()) = %s', room && room.alias(message.from()))
  ...
  if (room) {
    ...
    if (rule) {
      if (matchString(sender, rule.babies) && matchString(message.content(), rule.keywords)) {
        delay(ms(rule.delay)).then(async () => {
